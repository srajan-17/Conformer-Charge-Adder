...
def create_data_files(in_dest, out):
    f = open(in_dest, "r").readlines()
    cutoff_list = []
    for line_index in range(len(f)):
        for word in f[line_index].split():
            if word == "@<TRIPOS>ATOM":
                cutoff_list.append(line_index)
            elif word == "@<TRIPOS>BOND":
                cutoff_list.append(line_index)
    for i in range(len(cutoff_list) / 2):
        output_data = open(out + "/data" + str(i + 1) + "d.tpl", "w")
        data_to_write = f[cutoff_list[(i * 2)] + 1:cutoff_list[((i * 2) + 1)]]
...
def getdatafiles(out):
    datafiles = [f for f in os.listdir(out) if f.endswith("d.tpl")]
    return len(datafiles)


# write_charges_file() writes the charges data to the final output file
def write_charges_file(out, foldercode, phno):
    charges_output_file = open(out + "/charges_sum.tpl", "w")
    ...
    total_sum = 0
    for i in range(1, getdatafiles(out) + 1):
        data = numpy.loadtxt(out + "/data" + str(i) + "d.tpl", dtype=float, usecols=[8])
...
if __name__ == '__main__':
    ...
    input_destination = str(os.getcwd()) + "/tpl_" + "/" + str(arguments.tl.upper()) + "/pH_" + str(
        arguments.pH.upper()) + "/" + str(arguments.tl.upper()) + "_pH_" + str(
        arguments.pH.upper()) + "-charged_output.mol2"
    output_destination = str(os.getcwd()) + "/tpl_" + "/" + str(arguments.tl.upper()) + "/pH_" + str(
        arguments.pH.upper()) + "/param"

    # If the output place doesn't exist, make it exist
